...
`;function fn(){let e=document.getElementById(Bt);return e||(e=document.createElement("style"),e.id=Bt,document.head.appendChild(e)),e.textContent=Ir,()=>{document.getElementById(Bt)===e&&e.remove()}}function Pr(e){wo(e),so(e);let t=fn(),n=qo(e),i=on(e),r=e.files.onRenamed(({oldPath:k,newPath:x})=>{fo(k,x)}),a=e.files.onLineShift(({path:k,fromLine:x,delta:T})=>{go(k,x,T)}),s=Nt(),l=async k=>{if(k){if(k.surface==="web"){if(!k.url||!k.text.trim())return;s.publish({kind:"web",url:k.url,snippet:k.text.trim().slice(0,200)})}else{if(!k.path||!k.page)return;let x=await e.workspace.getPdfPageText(k.path,k.page).catch(()=>null),T=Ho({path:k.path,page:k.page,text:k.text},x);if(!T)return;s.publish(T)}await e.workspace.revealOwnPanel("right_sidebar")}},f=e.interop.extensions.provide(ro,{id:"sideNotes.create",labelKey:"auto.94fd67ed6c0c",label:"Create SideNote from selection",surfaces:["pdf","web"],run:l}),b=e.commands.register({id:"highlight-selection",label:"Create SideNote from selection",labelKey:"auto.94fd67ed6c0c",hotkey:"Mod-Shift-h",sideEffect:"read",run:()=>{let k=e.getState().activePath,x=k&&de(k)==="pdf"?Wo(k):null;x&&l({surface:"pdf",...x})}});e.registerView("sideNotes.panel",an),e.registerView("sideNotes.flagged",pn),e.registerView("sideNotes.settings",cn);let h=ko(),g=jo(e);return()=>{r(),n(),i(),a(),f(),b(),h(),g(),t()}}var _r={register:Pr},Os=_r;export{Os as default,Pr as register};
